...
public class PlusInflixExpressionFolding implements Folding {
   ...
   * of some node in top
   * 
   * <p>isFoldable(n) := isNumericAdditionExpression(n)
  ...
  private void checkRequires(final ASTNode root) {
    ExceptionUtils.requiresNonNull(root, "Null root passed to PlusInflixExpressionFolding.fold");
    if (!(root instanceof CompilationUnit) && root.getParent() == null) {
      ExceptionUtils.throwRuntimeException(
          "Non-CompilationUnit root with no parent passed"
              +
              "to PlusInflixExpressionFolding.fold");
    }
  }
